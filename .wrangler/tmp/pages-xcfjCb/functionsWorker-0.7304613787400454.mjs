...
async function onRequestGet(context) {
  try {
    ...
    const todaysSales = sales.filter((s) => s.created_at.startsWith(todayStr)).reduce((sum, s) => sum + s.total_amount, 0);
    const todaysCashIn = cashFlow.filter((c) => c.transaction_type === "IN" && c.created_at.startsWith(todayStr)).reduce((sum, c) => sum + c.amount, 0);
    const todaysCashOut = cashFlow.filter((c) => c.transaction_type === "OUT" && c.created_at.startsWith(todayStr)).reduce((sum, c) => sum + c.amount, 0);
...
// ../.wrangler/tmp/pages-xcfjCb/functionsRoutes-0.6258705281625295.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-0GbXLT/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-0GbXLT/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7304613787400454.mjs.map
